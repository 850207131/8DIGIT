...
//describe:
//八数码问题广度优先搜索+启发式搜索(将牌不在位的距离和+3S(n))求解
#include<iostream>
#include<queue>
#include<bitset>
#include<math.h>
...
static int getCost(string s)
{
	...
	for(int i=0;i<8;i++){
		...
		char next_num=(char)(((int)s[i]+1)%8);
		//顺时针方向
		if(s[i]=='0'&&i!=5)
			cost+=3;
		else if(s_row==0&&s_col+1<=2&&s[s_row*3+s_col+1]!=next_num)
			cost+=6;
		else if(s_row==2&&s_col-1>=0&&s[s_row*3+s_col-1]!=next_num)
			cost+=6;
		else if(s_col==0&&s_row-1>=0&&s[(s_row-1)*3+s_col]!=next_num)
			cost+=6;
		else if(s_col==2&&s_row+1<=2&&s[(s_row+1)*3+s_col]!=next_num)
			cost+=6;
	}
	return cost;
}
...
string AStar3(string init, int& c_n,int& e_n)
{
